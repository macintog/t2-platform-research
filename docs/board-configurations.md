...
`pressure`. LAS denotes the low-power always-listening service. An absent
DeviceTree child establishes a configuration difference, not the absence of a
silicon block.
...
The same records are available in [boards.json](../data/boards.json).
...
See [Artifacts and method](artifacts-and-method.md#reproducing-the-inventory)
for extraction commands and [Host coordination](host-coordination.md) for the
