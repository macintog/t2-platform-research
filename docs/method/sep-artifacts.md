...
The [shared platform artifact reference](platform-artifacts.md#firmware-identities)
...
| Analyzed decrypted SEP lookup payload (4,182,016 bytes) | `1b29e87948c08ddd49a35137e8857892f7c5101fbe1de988dac357a9fa7cf66f` |
| Reconstructed `sks` Mach-O (417,792 bytes) | `57178e6692f2599110eee5a01da24629cd2d5eacf474df09c53a0ae704f7bacd` |
...
[Identity and authorization](../sep/aks-identity-and-authorization.md).

These payload and `sks` rows belong to the retained `23P6068` comparator. The
analyzed lookup payload is also retained byte-for-byte under the J214K
`23P6068` family extraction, but that does not make it the earlier `23P2048`
version-4 artifact. The [version-4 analysis](../sep/aks-create-v4-j214k.md) records `sks`
UUID exactly, but neither its decrypted payload nor its reconstructed Mach-O
survives in the retained corpus, so no size or hash is available for either
file. Retained AppleDB metadata identifies the source IPSW as 730,089,773 bytes
with SHA-256
`eefcf1c7da03b7207fd36c6ea83ffd13efb512eb67e0503f2e6a6ae0faa1a057`.
Rechecking the missing trace requires the original analyzed files or a separately
authorized extraction, decryption, and reconstruction from that exact IPSW.
...
The corrected interpretations below come from the matched J152f reference work.
Separately, the [J214K contributor report](https://github.com/macintog/t2touch/pull/1)
reported one-machine version-4 creation, enrollment, verification, `sudo`, and
restart results while retaining the ordinary password fallback. That fallback
observation is an operating-system authentication property; it must not be
conflated with the failed experiment below that tried the account password as an
AKS activation credential.
