...
Offline analysis dated 2026-09-15. The primary evidence recorded by that
analysis was Apple's decrypted J214K 23P2048 `sks` module, UUID
`9d8aad8b-0dbc-1c37-95f9-cd0316e38d5a`. Addresses below use the reconstructed
`sks` Mach-O virtual address space. At analysis time, TEXT and initialized DATA
bytes were checked against the decrypted payload; only detached LINKEDIT
metadata was removed, and no instruction bytes were changed.

The repository retains this analysis, not the 23P2048 payload or reconstructed
Mach-O. Their byte sizes and hashes are therefore unavailable for an independent
recheck. The retained 23P6068 payload and `sks` are later comparators, not copies
of this artifact. No request was sent to hardware as part of this analysis.
...
[AKS request codec](../../reference-code/t2_aks_identity_create.py), rather
...
## Evidence boundaries and reported hardware qualification

The address-level claims in this chapter are offline firmware analysis. A
separate [credited contributor report](https://github.com/macintog/t2touch/pull/1)
tested an explicit version-4 implementation on one MacBookPro16,2 / J214K with
bridgeOS `23P2048` and kernel `7.1.8-arch1-Watanare-T2-3-t2`. It reported
creation and activation, one enrollment, matching through the ordinary client,
fingerprint-only `sudo`, successful matching after restart, and retained
password fallback. Negative-finger controls, separate graphical lock and
PolicyKit transactions, additional fingerprints, and deletion were not tested.

Those observations qualify one implementation and machine; they do not turn
the dated static trace into a hardware trace. In particular, “password fallback
retained” means the ordinary password authentication path remained available.
It is not evidence that the account password was the AKS creation input or could
activate the exported identity.
...
[Identity and authorization](aks-identity-and-authorization.md):
...
- The raw item1 pointer and length reach the 32-byte derivation wrapper
  `0xd1a7a` at `0xda6ca`. A separate Boolean at `0xda52e..0xda542` is true
  when item1 is missing, empty, **or has a first byte of zero**. Its construction
  does not rewrite item1: the raw pointer and length still reach the derivation
  call. The retained v4 note does not identify the Boolean's local, subsequent
  reads, or branch effects, so it cannot be classified as harmless bookkeeping
  or assigned a more specific semantic name.
...
The retained matched version-5 comparator has the same predicate at
`0xe32b0..0xe32c4`, stored at stack offset `0x2c`. In that later binary it is
substantive: the true branch sets object flag `0x2` at `0xe3492..0xe349e`, is
passed to classification helpers at `0xe3708` and through `0xe37fc`, and skips
one mask-`0x100` per-operation candidate at `0xe3726..0xe372a`. At `0xe3852`,
true selects the `0xe1fac` wrapper with normalized item1 pointer and length;
false selects `0xe253c` with a generated 32-byte buffer and zero auxiliary
arguments. The adjacent mask table does not by itself supply a safe public name
for the filtered candidate. These version-5 consumers are a search guide, not
evidence that the unavailable version-4 binary has the same downstream graph.

The v4 predicate and independent raw-input derivation path are therefore exact;
the v4 Boolean's consumers remain unresolved. Recovering them requires the
23P2048 `sks` matching the UUID above, or an instruction-complete dump of
`0xda4a0` through its cleanup paths. Other static uncertainties include
platform-dependent KDF and persistence behavior below these helpers and the
returned KEK material. The separate hardware report establishes narrower
one-machine outcomes, not those internal semantics.
