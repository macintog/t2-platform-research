...
client lifetime, and recovery. The reference
...
t2touch's current product policy exposes five independent slots, Finger 1
through Finger 5. The recovered protocols do not establish five as a hardware
or service capacity. An integrator may choose another limit, but its labels
must remain stable: deleting one never renumbers survivors, and a successful
enrollment receives an unused label under the same operation lock that protects
the reconciled inventory. Under t2touch's policy, the next enrollment takes the
lowest vacancy, an empty inventory starts at Finger 1, and failed or cancelled
capture does not consume a slot.
...
The [T1 research reference](../method/sep-artifacts.md#research-credits) informed
