# Linux integration of SEP identity

[Research map](../README.md#fingerprint) · [Evidence catalog](../../catalog/README.md)
...
The [research reference](README.md) covers the internal SEP services,
...
It writes a pending secret and returns its digest only after syncing both the
pending generation and its parent directory. After export, call `commit()` with
the saved keybag, its digest, the secret digest, and the bag UUID. The store
writes mode-`0600` files and publishes the generation by renaming its directory.
If the rename succeeds but its parent sync fails, retrying `commit()` verifies
the published bytes and retries that sync before reporting success. The caller's
secret and keybag buffers are wiped when these methods return.
...
t2touch's product policy exposes five neutral slots, `Finger 1` through
`Finger 5`, backed by private identity UUIDs; the recovered protocols do not
establish five as a hardware or service capacity. Whatever limit an adapter
chooses, deleting one identity must not renumber any survivor and a slot label
must stay with its identity while that identity exists. Under t2touch's policy,
the next successful enrollment takes the lowest vacant slot. Labels do not
establish which physical finger the person used.

The [integration contracts](fingerprint-integration.md) describe
