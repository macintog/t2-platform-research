    ...
    def published(
        ...
        *,
        ...
        try:
            ...
            _wipe(manifest)
        # A prior commit may have completed rename() but failed the parent
        # directory sync. Reconciliation must close that durability boundary
        # before reporting the already-published generation as successful.
        _sync_directory(self.root)
        return ActivationBundle(
            generation=self.operation_id,
            directory=self.final_directory,
